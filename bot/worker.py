...
async def dl_link(event):
    ...
    if WORKING or QUEUE:
        ...
        return await event.reply(f"**✅ ᴀᴅᴅᴇᴅ {link} ɪɴ ǫᴜᴇᴜᴇ**")
    WORKING.append(1)
    s = dt.now()
    xxx = await event.reply("**⚡ ᴅᴏᴡɴʟᴏᴀᴅɪɴɢ...**")
    try:
    ...
    ttt = time.time()
    await nn.delete()
    nnn = await xxx.client.send_message(xxx.chat_id, "**⚡ ᴜᴘʟᴏᴀᴅɪɴɢ...**")
    with open(out, "rb") as f:
        ok = await upload_file(
            ...
            progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                progress(d, t, nnn, ttt, "**⚡ ᴜᴘʟᴏᴀᴅɪɴɢ...**")
            ),
        )
    await nnn.delete()
...
async def encod(event):
    try:
        ...
        if WORKING or QUEUE:
            time.sleep(2)
            xxx = await event.reply("**ᴀᴅᴅɪɴɢ ᴛᴏ ǫᴜᴇᴜᴇ...**")
        ...
        try:
            if hasattr(event.media, "document"):
                ...
                dl = await event.client.download_media(
                    ...
                    dir,
                    progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                        progress(d, t, xxx, ttt, f"**⚡ ᴅᴏᴡɴʟᴏᴀᴅɪɴɢ**\n__{filename}__")
                    ),
                )
        except Exception as er:
            WORKING.clear()
        ...
        nn = await e.edit(
            ...
            buttons=[
                ...
                [Button.inline("❌ ᴄᴀɴᴄᴇʟ", data=f"skip{wah}")],
            ],
        )
        cmd = f"""ffmpeg -i "{dl}" {ffmpegcode[0]} "{out}" -y"""
        process = await asyncio.create_subprocess_shell(
        ...
        ttt = time.time()
        await nn.delete()
        nnn = await e.client.send_message(e.chat_id, "**⚡ ᴜᴘʟᴏᴀᴅɪɴɢ...**")
        with open(out, "rb") as f:
            ok = await upload_file(
                client=e.client,
                file=f,
                name=out,
                progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                    progress(d, t, nnn, ttt, f"**⚡ ᴜᴘʟᴏᴀᴅɪɴɢ**\n__{out.replace(f'encode/', '')}__")
                ),
            )
        await nnn.delete()
        org = int(Path(dl).stat().st_size)
